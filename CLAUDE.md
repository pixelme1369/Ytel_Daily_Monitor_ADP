...
| Agent Report Cards | New bottom section `#sec-report-cards` (nav link "Agent Report Cards"), last section on the page — card `#reportCardsCard` with a single button "⬇ Download All Agent Cards (ZIP)" → `exportAgentReportCards()`. **Closers only** — `rows` filters `window._agentAllRows` to `CLOSERS.has(a.name.toLowerCase())`, so Openers/Retention never get a card regardless of role-editor state (user preference, not tied to the role-filter buttons elsewhere in the app); the card's role line always reads "Closer" since only closers reach the render step. `window._agentAllRows` already excludes `TERMINATED`/`INACTIVE_RECENT`, same as every other agent view. Uses `window._summary.dateStr` for the report-period label (the currently selected date range, not a fixed daily window). For each closer, builds a one-page letter-size PDF via jsPDF: agent name, "Closer" tag, report period, then Total Calls / Inbound (unique) / Outbound (unique) / Transfers Received / Enrolled / Enrolled Debt / Conv% (enr/>2min calls — same `pct(a.enr,a.gt2m)` formula as the Agent Performance table, `—` if either side is 0). Transfers Received is looked up from `window._receivedTransferRows` (the same data backing the "Correct Transfers Received by Agent" card and Agent Management Board), matched by lowercased agent name, `0` if the agent has no entry there. Filename is the agent name slugified (`toLowerCase()`, non-alphanumeric → `_`) + `.pdf`. All per-agent PDFs are bundled into one ZIP via JSZip (`agent_report_cards_<dateStr-slugified>.zip`) and downloaded via a Blob URL — one PDF per agent (not a combined multi-page PDF) so a single agent's card can be shared 1:1 in Slack without exposing everyone else's numbers. Shows a toast and no-ops if there are no closers in the current data/range or the CDN libraries failed to load. |
