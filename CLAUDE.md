...
- Each agent row (`agentRows` in `buildDashboard`) carries `inboundPhones`/`outboundPhones` arrays of `{phone, crmStatus, ytelStatus, debt, recording, sec}` — `debt` comes from `enrolledPhoneAgent[phone].debt` (0/— if the phone never enrolled, since Enrolled Debt only applies to enrolled phones)
