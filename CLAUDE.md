...
- `agentMap[name].phoneRecordings` — `{phone: [{sec, recording, crmStatus}, ...]}`, one entry per qualifying call (≥1200 sec), populated alongside `phoneMaxSec`; `recording` comes from `r._recording` (parsed from `recording_location` column), `crmStatus` from `r._crmStatus` (parsed from `CRM Status` column)
...
- `renderLongNoDeal(rows)` is a top-level function (not nested in `buildDashboard`, since the dropdown must call it after initial render): for each agent it filters `phoneMaxSec` keys ≥ the current threshold, subtracts the agent's enrolled phones (`agentEnrollPhones[name.toLowerCase()]`) to get `longNoConvert`, then splits that into `pitchReadyPhones` (any qualifying call has `crmStatus` matching `PITCH_READY_STATUS = 'pitch ready - scheduled meeting'`, case-insensitive) vs `otherPhones` (everyone else) — mutually exclusive
...
- Card table (`longNoDealCard`/`longNoDealBody`): Agent | Long Calls | Converted | Pitch Ready | Not Converted, plus a TOTAL row summing all four numeric columns across agents — sorted by total not-converted (`longNoConvert.length`) descending, only agents with `longNoConvert.length>0` shown
- Clicking the Pitch Ready or Not Converted count opens the shared phone modal via `showFlaggedPhones(records, label)` (mirrors `showEnrolledPhones`, same modal DOM; `label` already includes the section name, e.g. `"<agent> — Pitch Ready (Scheduled Meeting)"`). Each row shows the phone, call duration, an inline `<audio controls>` player sourced from `recording.recording`, and a "⬇ Download recording" link (`download` attribute on an `<a href="...">`). Phones with no recording show "No recording available" instead of a player.
...
- **Export CSV** button (`⬇ CSV` next to the threshold dropdown) calls `exportLongNoDeal()`, which flattens `window._longNoDealFlagged` (set at the end of every `renderLongNoDeal()` run) into `Agent,Phone Number,CRM Status` rows (CRM Status column is `Pitch Ready - Scheduled Meeting` or blank) and downloads `long_calls_no_deal.csv` — reflects whatever threshold is currently selected
