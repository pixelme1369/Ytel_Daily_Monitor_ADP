...
Per user request ("on any count of calls when I click I wanna see the recording if available, Ytel status and CRM status with the debt amount") — clarified via a screenshot to mean **every** clickable count of calls, not just Enrolled — every bare or non-clickable "count of calls" cell across the dashboard now opens the same full-detail `showPhoneCrmList()` modal (recording player/download, Ytel status pill, CRM Status pill, Enrolled Debt) already used by the Inbound/Outbound columns. This landed in two rounds:

**Round 1 — Agent Performance table main rows.** Per-agent time-bracket counts (Short% ≤30s, `<2 min`, `1–2 min`, `5–10 min`, `10–15 min`, `15–20 min`, `20–30 min`, `30+ min`) and the `Enrolled` column, on both each agent's main row and the TOTAL row.

- `funnelCellClick(val, denom, color, records, label)` — top-level function next to `funnelCell` (the plain, non-clickable renderer, still used standalone anywhere records aren't tracked). Renders identical output to `funnelCell` but wraps it in a `showPhoneCrmList()`-triggering `<span class="enroll-click">` when `records` is non-empty; falls back to plain `funnelCell` output otherwise so a zero-count cell stays inert (and doesn't block the row's expand/collapse click, since `event.stopPropagation()` lives on the inner span, not the `<td>`)
- `agentMap[name].funnelRecords` (and the matching `agentDirMap`/`agentCampDataMap`/hourly-map copies) gained two new bracket keys, `short` and `r1to2m`, alongside the pre-existing `lt2m`/`r5to10`/`r10to15`/`r15to20`/`r20to30`/`gt30m` — previously these two brackets were counted (`d.short`/`d.r1to2m`) but had no per-call record array, so they couldn't be clicked
- **Bug fixed alongside this: every `funnelRecords` entry's `debt` field was hardcoded to `0`.** The per-row record builder (the shared `mkFunnelRec()` closure, replacing three near-duplicate inline object literals and a separately-duplicated local `funnelRec()` closure) now looks up `rowDebt = enrolledPhoneAgent[r._phone]?.debt || 0` once per row and uses it for every bracket push — so a bracket popup for a phone that actually enrolled now shows its real Enrolled Debt instead of always `—`
- `agentRows[].enrolledRecords` — one record per phone in `agentEnrollPhones[name]`, resolving the matching call in `agentPhoneCallLog[name+'||'+phone]` by timestamp (`c.ts===enrolledPhoneAgent[phone].ts`, i.e. the actual enrolling call; falls back to the call log's last entry if no exact match) for `recording`/`crmStatus`/`ytelStatus`/`dir`/`sec`, plus `debt`/`enrolledDate` from `enrolledPhoneAgent[phone]` directly. The Agent Performance table's `Enrolled` cell (main row and TOTAL row) opens `showPhoneCrmList()` with these records instead of the old `toggleEl()` inline plain-phone-list toggle.

**Round 2 — everywhere else with the same gap.**

- **Agent Performance table hour sub-rows**: the shared `[h,hd,...(hc?[hc]:[])].forEach(x=>{...})` accumulator (feeds `agentHourMap`/`agentHourDirMap`/`agentHourCampMap`) now builds the same `funnelRecords` (short/lt2m/r1to2m/r5to10/r10to15/r15to20/r20to30/gt30m) per hour bucket, and the enrollment-hour-crediting loop now also pushes an `enrHourRecords` array (one record per enrolled phone credited to that hour, same timestamp-match-to-`enrolledPhoneAgent` pattern as `agentRows[].enrolledRecords`). Every hour sub-row's bracket cells and `Enrolled` cell are now clickable, same as the main row.
- **Campaign/Queue Breakdown table** (`campMap`/`campDirMap`/`campAgentMap`/`campAgentDirMap`, both the initial `renderCampBody(campMap)` call and `filterCampTable()`'s locally-rebuilt filtered maps): gained the same `funnelRecords` (short/lt2m/r5to10/r10to15/r15to20/r20to30/gt30m — no `r1to2m`, since this table never had that column) on campaign rows, agent sub-rows, and the TOTAL row (aggregated via `campRows.flatMap`). The `Short%` cell (previously plain `count <span>%</span>`, not even a `funnelCell`) and all bracket cells are now clickable; the `Enrolled` column switched from `showEnrolledPhones` (bare phone list) to `showPhoneCrmList()` with real records.
- **A phone-level call log, `phoneCallLog`** (built once in `buildDashboard()`, keyed by phone number across *all* agents/campaigns, not just one agent's calls) was added specifically to support "Enrolled" clicks on tables that aggregate across agents (Campaign Breakdown, Openers) where the existing `agentPhoneCallLog[name+'||'+phone]` agent-scoped log doesn't apply. Exposed as `window._phoneCallLog` (and `window._enrolledPhoneAgent`, `enrolledPhoneAgent` re-exposed the same way) immediately after being built — **before** any of the render calls later in the same `buildDashboard()` pass (Openers, Campaign table) — precisely so those top-level render functions, which run outside `buildDashboard()`'s closure and can only reach this data via `window`, see fully-populated data on the very first render, not just on later filter interactions.
- `buildPhoneRecordsFromSet(phoneSet, phoneLog, enrolledMap)` — new top-level function next to `funnelCellClick`. Turns a bare `Set` of enrolled phone numbers into full `showPhoneCrmList()`-ready records: for each phone, finds the call in `phoneLog` matching `enrolledMap[phone].ts` (the actual enrolling call; falls back to the log's last entry) for `recording`/`crmStatus`/`ytelStatus`/`dir`/`sec`, plus `debt`/`enrolledDate` from `enrolledMap[phone]` directly. `phoneLog`/`enrolledMap` default to `window._phoneCallLog`/`window._enrolledPhoneAgent` when omitted, so top-level callers (`filterCampTable()`, `renderOpenerTable()`) can call it with no args.
- **Correct Transfers Received** (`receivedTransferRows`): `records` (backing the `Transfers Received` count) now carries a `debt` field per call; that count and the `Not Enrolled` count both switched from `showFlaggedPhones` to `showPhoneCrmList` so debt actually renders. `Enrolled` switched from `showEnrolledPhones` to a new `enrolledRecords` array (via `buildPhoneRecordsFromSet`). The Agent Management Board's copy of the "Transfers Received" count (`renderTransferLeaderboard()`, which reuses the same `records`) got the same `showFlaggedPhones`→`showPhoneCrmList` swap.
- **Openers Transfer Breakdown**: the per-agent/campaign row's `Enrolled` cell (`renderOpenerTable()`'s `enrollCell`) switched from `showEnrolledPhones` to `showPhoneCrmList(buildPhoneRecordsFromSet(s.enrolledPhones))`. The row's per-bracket Transfer-count popup (`rangeCell`, e.g. "3 Transfer") switched from `showFlaggedPhones` to `showPhoneCrmList` with debt enriched onto each record via `window._enrolledPhoneAgent`, since the underlying `recs_XtoY` records are one-per-CLtrns-call and must keep showing that specific transfer call's own recording (not the enrolling call's), unlike the Set-based `Enrolled` cell.
- **Openers Summary Table** (rows = time brackets, `#openerSummaryBody`): both `Total Transfers` and `Total Enrolled` columns were entirely non-clickable plain numbers before this round. Fixed by adding a new `enrRecs_XtoY` array (and `enrRecs_short` for the Dead ≤30s row, which previously tracked no per-call detail for its enrolled count at all) alongside the pre-existing `recs_XtoY` transfer-call arrays, aggregated the same way through `addS()`/`zeroTot()`/`emptyS()`. Both columns are now clickable on every bracket row and the TOTAL row (aggregated via `brackets.flatMap`); `enrRecs` records get `debt`/`enrolledDate` enriched at render time via a local `enrichEnr()` helper reading `window._enrolledPhoneAgent`, since the accumulation loop itself only had per-call fields (phone/sec/recording/crmStatus/ytelStatus/dir), not debt.
- `showEnrolledPhones()` (the old bare-phone-list modal function) had zero remaining callers after this round and was deleted; `closeEnrolledPhones()` (the shared modal's close button) is unrelated and still in use.
- **Still not upgraded** (same "plain number, no click-through" gap, not reported as needed yet): the Agent Call Funnel table's own bracket cells were already clickable before this work (see "Agent Call Funnel Table" section) and are unaffected; the Openers per-agent breakdown table's `Total Calls`/`Short%`/individual duration-bracket-call counts outside of the Transfer/Enrolled cells covered above remain plain numbers, since those aren't really "a count of calls that happened" in the same sense (they're just row totals, not phone-flaggable events) — flag if this assumption is wrong.
