...
- Card table (`longNoDealCard`/`longNoDealBody`): Agent | Long Calls | Converted | Conv% | Not Converted — sorted by Not Converted descending, only agents with `longNoConvert.length>0` shown. Conv% = `pct(converted, a.total)` (converted long calls ÷ total long calls, e.g. 1 converted of 8 long calls = 12.5%)
