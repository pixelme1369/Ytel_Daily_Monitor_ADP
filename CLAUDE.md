...
- `agentPhoneDetail[name][phone]` — built in the same per-row loop as `agentMap`/`agentDirMap` (line ~2210), keeps the CRM Status (`r._crmStatus`) AND Ytel status (`r._status`, from the `status` CSV/XLSX column, e.g. SALE/DNC/DROP/CLtrns) from whichever row has the latest `r._ts` for that phone
- Each agent row (`agentRows` in `buildDashboard`) carries `inboundPhones`/`outboundPhones` arrays of `{phone, crmStatus, ytelStatus, debt, recording, sec}` — `debt` comes from `enrolledPhoneAgent[phone].debt` (0/— if the phone never enrolled, since Enrolled Debt only applies to enrolled phones); `recording`/`sec` come from `agentPhoneFirstDir[name+'||'+phone]` (extended to also carry `recording:r._recording`/`sec:r._sec` off the same first-call row that determines the direction bucket, so the audio played matches the specific inbound/outbound call being counted, not just the phone's latest call)
