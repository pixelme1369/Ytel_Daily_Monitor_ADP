...
| Dialed After DNC | KPI next to "DNC Still Dialed" = unique phones with an **outbound** call strictly after their first DNC disposition in the range (`firstDncTs` per phone; inbound callbacks excluded) — the true compliance-violation count; phones kept in `window._dialedAfterDncPhones`; included in the Copy Summary digest |
